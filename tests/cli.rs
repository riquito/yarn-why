...
// `string-width-cjs` is an alias for `string-width`, so both the descriptor
// and the dependency embed a second `name@version` pair.
const YARN_LOCK_V8_WITH_ALIASED_DEP: &str = r#"# This file is generated by running "yarn install" inside your project.
# Manual changes might be lost - proceed with caution!
...
"emoji-regex@npm:^8.0.0":
  ...
  checksum: 10c0/b6053ad39951c4cf71f0c05f7ac6be4a0b60d43de1c93b0d95d745b0bcdd672c7c5cec8827b0cf9d59a4635a9b8e4c7c95b0d9b17ea0c8f6a6f27b3e3b7e1e0f
  languageName: node
  linkType: hard
"#;
...
#[test]
fn it_finds_a_package_reached_through_an_aliased_dep() {
    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();

    let assert = cmd
        .args(["emoji-regex"])
        .write_stdin(YARN_LOCK_V8_WITH_ALIASED_DEP)
        .assert();

    assert.success().stdout(
        r#"├─ foo@0.0.0-use.local (via .)
│  └─ string-width-cjs@4.2.3 (via ^4.2.0)
│     └─ emoji-regex@8.0.0 (via ^8.0.0)
│
└─ string-width@4.2.3 (via ^4.1.0)
   └─ emoji-regex@8.0.0 (via ^8.0.0)
"#,
    );
}
...
#[test]
fn it_prints_records_of_an_aliased_dep_without_the_protocol() {
    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();
    ...
    assert.success().stdout(
        r#"{"name":"foo","version":"0.0.0-use.local","descriptor":"."}
{"name":"string-width-cjs","version":"4.2.3","descriptor":"^4.2.0"}
{"name":"string-width-cjs","version":"4.2.3","descriptor":"^4.1.0"}
{"name":"emoji-regex","version":"8.0.0","descriptor":"^8.0.0"}
"#,
    );
}
